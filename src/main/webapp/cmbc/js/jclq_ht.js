	...
	$("[num='open']").each(function(index) {
		...
		$(this).find(".jc-table.on").each(function(index) {
			...
			$(this).find("td.on").each(function(aa){
				...
				if( ptype == "01"){
					result = "（让球）</br>"+$(this).html();
				}else if(ptype == "03"){
					result = "（胜分差）</br>"+$(this).html();
				}else if(ptype == "04"){
					result = "（大小）</br>"+ $(this).html();
				}
				var data_dit = $(this).attr("data-dit");
				htmldiv = htmldiv + '<span class="jc-list-item-dw on" data-type="'+ptype+'" data-dit="'+data_dit+'" onClick="delClick(this)" >'+ result +'</span>';
			});
			htmldiv = htmldiv + '</div>';
		});
